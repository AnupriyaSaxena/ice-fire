...
class Pagination extends Component {
    ...
    return pageNumbers.map(number => {
      return (
        <span
          ...
          onClick={() => this.showCurrentContent(number)}
        >
          {number}
        </span>
      );
  ...
  nextPage = () => {
    this.setState(
      prevState => ({ current_page: prevState.current_page + 1 }),
      () => {
    ...
    return (
      <div className="row">
        <div className="pagination">
          <span
            className={current_page === 1 ? "disabled" : ""}
          ...
          {this.getPageNumbers()}
          <span
            className={current_page === this.getTotalPages() ? "disabled" : ""}
